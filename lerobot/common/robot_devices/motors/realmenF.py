...
class FollowRealmenMotorBus:
    ...
    def __init__(
        ...
        if motor_util.node_initialize==False:
            rospy.init_node("FollowRealmenMotorBus",anonymous=True)
            motor_util.node_initialize=True
        self.control_interval = 1/30.0  # Control interval (s)
        self.control_rate = rospy.Rate(1 / self.control_interval)  # 30Hz
        ...
        while not rospy.is_shutdown():
            try:
                ...
                if self.is_sim == False:
                    ...
                    self.jointTrajCtrl(self.write_state["Goal_Position"])   
    ...
    def connect(self):
        ...
        self.gripper_motors_bus.write("Mode", 0)
        # Set P_Coefficient to lower value to avoid shakiness (Default is 32)
        self.gripper_motors_bus.write("P_Coefficient", 16)
        # Set I_Coefficient and D_Coefficient to default value 0 and 32
        self.gripper_motors_bus.write("I_Coefficient", 0)
        self.gripper_motors_bus.write("D_Coefficient", 32)
        # Close the write lock so that Maximum_Acceleration gets written to EPROM address,
        # which is mandatory for Maximum_Acceleration to take effect after rebooting.
        self.gripper_motors_bus.write("Lock", 0)
        # Set Maximum_Acceleration to 254 to speedup acceleration and deceleration of
        # the motors. Note: this configuration is not in the official STS3215 Memory Table
        self.gripper_motors_bus.write("Maximum_Acceleration", 254)
        self.gripper_motors_bus.write("Acceleration", 254)
        while not rospy.is_shutdown():
    ...
    def reset_position(self,cur_joint_pos):
        ...
        step  =  convert_degrees_to_steps(self.home_offset[-1],self.gripper_model) 
                # time_s = time.time()
        self.gripper_motors_bus.write("Goal_Position",step ) 
        self.control_rate.sleep()
        if self.is_sim == False:
    ...
    def read(self,data_name,motor_names: str | list[str] | None = None): 
        ...
        if data_name not in self.read_state.keys():
            ...
            if data_name == "Present_Position":
                read_gripper = self.gripper_motors_bus.read("Present_Position")
                gripper_q = step2degree(read_gripper,self.gripper_model)*math.pi/180.0
                self.read_state[data_name] = self.joint_actual_pos
                self.read_state[data_name].append(gripper_q[0])
    ...
    def write(self,data_name,values: int | float | np.ndarray, motor_names: str | list[str] | None = None): 
        ...
        if self.write_state["Goal_Position"]!=None:         
            ...
            if actions >= 1:
                ...
                self.gripper_motors_bus.write("Goal_Position",step ) 
                if self.is_sim == False:
                    self.jointPosCtrl(self.write_state["Goal_Position"])
